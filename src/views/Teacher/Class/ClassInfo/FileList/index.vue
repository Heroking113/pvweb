  ...
  <div>
    ...
    <el-table-column fixed="right" label="操作" width="400" align="center">
      <template slot-scope="scope">
        ...
        <el-button @click="showDeleteFile(scope.row)" type="primary" size="mini" class="list-operation-button">删除文件</el-button>
      </template>
    </el-table-column>
    </el-table>
    <el-dialog title="上传文件" :visible.sync="isshowUpFileDialog" center style="width:1000px">
      ...
      <div>
        <span>文件名：</span>
        <el-input v-model="fileForm.name" class="input-file-name"></el-input>
      </div>
      <div>
        <span>文件描述：</span>
        <el-input v-model="fileForm.description" class="input-file-description"></el-input>
      </div>
    ...
    <span slot="footer" class="dialog-footer">
      <el-button @click="isshowDeleteFile=false">取消</el-button>
...
export default {
  ...
  methods: {
    ...
    */
    getFileList() {
      let params = { chapter_id: this.$store.state.chapter_id }
      this.axios.get("/files/files/", params).then(resp => {
        if (resp.result) {
    ...
    */
    ...
    handleChange(file, fileList) {
      ...
      this.fileForm.name = name[0]
    },
    /*
    上传文件
    */
    uploadFile() {
      if (!this.fileForm.description) {
        this.$message.warning("请填写文件描述！");
      } else {
    ...
    */
    showDeleteFile(row) {
      this.deleteFileElem = row.id
      this.isshowDeleteFile = true
    },
    /*
    删除文件
    */
    deleteFile() {
      this.axios.delete("/files/files/" + this.deleteFileElem + "/").then(resp => {
        if (resp.result) {
